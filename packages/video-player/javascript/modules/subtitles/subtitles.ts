...
export function validateRemoteTextTrackOptions(opts: RemoteTextTrackOptions): void {
    if (isAutoGenerate(opts)) {
        ...
        if (conflicting.length > 0) {
            throw new Error(
                `Invalid RemoteTextTrackOptions: Cannot specify [${conflicting.join(', ')}] when autoGenerate is true.`
            );
        }
        ...
        if (opts.showAutoGenerated !== undefined && typeof opts.showAutoGenerated !== 'boolean') {
            throw new Error(
                `Invalid RemoteTextTrackOptions: showAutoGenerated must be a boolean.`
            );
        }
    }
    ...
    if (hasSrc(opts) && (opts.maxWordsPerLine || opts.highlightWords)) {
        ...
        if (!isTranscript) {
            throw new Error(
                `Invalid RemoteTextTrackOptions: src must be a transcript URL when using maxWordsPerLine or highlightWords.`
            );
        }
    }
    ...
    if (isAutoGenerate(opts) && opts.showAutoGenerated === false) {
        if (!isTranslate(opts) || opts.translations.length === 0) {
            throw new Error(
                `Invalid RemoteTextTrackOptions: showAutoGenerated: false only makes sense when translations are provided.`
            );
        }
    }
    ...
    if (isTranslate(opts)) {
        if (!isAutoGenerate(opts)) {
            throw new Error(
                `Invalid RemoteTextTrackOptions: translations can only be used with autoGenerate.`
            );
        }

        if (opts.maxWordsPerLine || opts.highlightWords) {
            throw new Error(
                `Invalid RemoteTextTrackOptions: translations cannot be used with maxWordsPerLine or highlightWords.`
            );
        }

        if (!Array.isArray(opts.translations)) {
            throw new Error(`Invalid translations option: must be an array.`);
        }

        const defaultCount = opts.translations.filter(t => t.default).length + ((opts.default) ? 1 : 0);
        if (defaultCount > 1) {
            throw new Error(`Invalid translations option: only one entry can have default set to true.`);
        }

        opts.translations.forEach((t, idx) => {
            if (!t.langCode || typeof t.langCode !== 'string') {
                throw new Error(`Invalid translations option at index ${idx}: langCode is required and must be a string.`);
            }
            if (!languageCodes[t.langCode.toLowerCase()]) {
                throw new Error(`Invalid translations option at index ${idx}: langCode "${t.langCode}" is not supported.`);
            }
            if (t.label && typeof t.label !== 'string') {
                throw new Error(`Invalid translations option at index ${idx}: label must be a string.`);
            }
            if (t.default && typeof t.default !== 'boolean') {
                throw new Error(`Invalid translations option at index ${idx}: default must be a boolean.`);
            }
        });
    }
}
                ...
                if (showAutoGenerated) {
                    ...
                    const initTrack = async () => {
                        try {
                            ...
                            player.log.warn(`Base subtitle setup failed: ${err instanceof Error ? err.message : String(err)}`);
                ...
                if (hasTranslations) {
                    const initTranslations = () => {
                        try {
                            ...
                            player.log.warn(`Translation tracks setup failed: ${err instanceof Error ? err.message : String(err)}`);
                        }
                    };
